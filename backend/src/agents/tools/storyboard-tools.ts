...
export function createStoryboardTools(episodeId: number, dramaId: number) {
  const readStoryboardContext = createTool({
    ...
    execute: async () => {
      ...
      const existingStoryboardsMapped = await Promise.all(
        existingStoryboards
      ...
      const payload = {
        ...
        characters,
        scenes,
        existing_storyboards: existingStoryboardsMapped,
      }
  ...
  const saveStoryboards = createTool({
    ...
    execute: async ({ storyboards }) => {
      ...
      logTaskProgress('StoryboardTool', 'save-begin', {
        ...
        shotNumbers: storyboards.map((sb: { shot_number: number }) => sb.shot_number).join(','),
      })
      ...
      for (const sb of storyboards) {
        ...
        await syncStoryboardCharacters(Number(newId), sb.character_ids || [])
  ...
  const updateStoryboard = createTool({
    ...
    execute: async ({ storyboard_id, ...fields }) => {
      ...
      const charIdsForValidation = 'character_ids' in fields
    ...
    execute: async ({ shots, rows, cols, mode }) => {
      ...
      if (mode === 'multi_ref') {
        ...
        const payload = {
          ...
          cell_prompts: shots.map((s: { shot_number: number; description: string; shot_type?: string }) => ({
      ...
      if (mode === 'first_last') {
        ...
        const payload = {
          grid_prompt: `${shots.length}个镜头首尾帧拼图，${shots.map((s: { description: string }) => s.description).join(' | ')}，电影级画面，专业摄影，${rows}行${cols}列风格统一`,
      ...
      // first_frame mode
      const cellPrompts = shots.slice(0, rows * cols).map((s: { shot_number: number; description: string; shot_type?: string }) => ({
        shot_number: s.shot_number,
        frame_type: 'first_frame',
        prompt: `电影级高质量首帧，${s.description}，${s.shot_type || ''}，专业摄影，${rows}x${cols} 宫格风格统一`,
      }))
      const payload = {
        grid_prompt: `${shots.length}个镜头首帧拼图，${shots.map((s: { description: string }) => s.description).join(' | ')}，电影级画面，专业摄影，${rows}行${cols}列风格统一`,
        cell_prompts: cellPrompts,
      }
      logTaskSuccess('StoryboardTool', 'grid-prompt-complete', { episodeId, cells: payload.cell_prompts.length, mode })
      return payload
